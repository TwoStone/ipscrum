...
/**
 * When
 * {@link com.google.gwt.user.client.ui.FlexTable.FlexCellFormatter#setRowSpan(int, int, int)}
 * is used, it causes unexpected shifting for cells on rows below spanned one, because
 * {@link FlexTable#prepareCell(int, int)} is naive and does not take into account fact of
 * spanning (well, often <code>setRowSpan()</code> is invoked after adding cells, so may
 * be we should say that <code>setRowSpan()</code> is guilty).
 * <p>
...
public final class FlexTableHelper {
		...
		final Set<Element> tdToRemove = new HashSet<Element>();
		{
			...
			for (int row = 0; row < rowCount; row++) {
				...
				for (int cell = 0; cell < cellCount; cell++) {
					final int colSpan =
							flexTable.getFlexCellFormatter().getColSpan(row, cell);
					final int rowSpan =
							flexTable.getFlexCellFormatter().getRowSpan(row, cell);
					if (rowSpan != 1) {
						final int column =
								FlexTableHelper.getColumnOfCell(flexTable, row, cell);
						for (int row2 = row + 1; row2 < row + rowSpan; row2++) {
							final int baseCell2 =
									FlexTableHelper.getCellOfColumn(flexTable, row2,
											column);
							for (int cell2 = baseCell2; cell2 < baseCell2 + colSpan; cell2++) {
								if (cell2 != -1) {
									 ...
									 */
									final Element td =
											flexTable.getCellFormatter().getElement(
													row2, cell2);
									tdToRemove.add(td);
								}
							}
	...
	private static int getColumnOfCell(final FlexTable flexTable, final int row,
			final int cell) {
		int column = 0;
	...
	private static int getCellOfColumn(final FlexTable flexTable, final int row,
			final int column) {
		final int cellCount = flexTable.getCellCount(row);
		int currentColumn = 0;
		for (int cell = 0; cell < cellCount; cell++) {
	...
	private static int getColSpan(final FlexTable flexTable, final int row,
			final int cell) {
