 ...
 */
public class GenericTicketView extends Composite implements IGenericTicketView {
	...
	private final Event<TypedEventArg<String>> changeDescriptionEvent =
			new Event<TypedEventArg<String>>();
	private final Event<TypedEventArg<StateType>> changeStateEvent =
			new Event<TypedEventArg<StateType>>();
	...
	public GenericTicketView() {
		...
		this.descriptionTextBox.addChangeHandler(new ChangeHandler() {
			...
			public void onChange(final ChangeEvent event) {
				GenericTicketView.this.changeDescriptionEvent.fire(
						GenericTicketView.this, new TypedEventArg<String>(
		...
		headPanel
				.setCellVerticalAlignment(stateGrid, HasVerticalAlignment.ALIGN_BOTTOM);
		headPanel.setCellHorizontalAlignment(stateGrid,
		...
		this.possibleStateComboBox =
				new TypedListBox<StateType>(new TypeRendere<StateType>() {
		...
		btnStatusWechseln.addClickHandler(new ClickHandler() {
			...
			public void onClick(final ClickEvent event) {
				try {
					final StateType selectedItem =
							GenericTicketView.this.possibleStateComboBox
									.getSelectedItem();
					GenericTicketView.this.changeStateEvent.fire(
							GenericTicketView.this, new TypedEventArg<StateType>(
									selectedItem));
		...
		stateGrid.getCellFormatter().setHorizontalAlignment(2, 1,
				HasHorizontalAlignment.ALIGN_RIGHT);
	...
	public <T extends Serializable> void add(final FieldTypeController<T> controller) {
		...
		this.flexTable.setWidget(rowCount, 0, new Label(controller.getField().getType()
				.getName()));
		this.flexTable.setWidget(rowCount, 1, controller.getWidget());
	}

	@Override
	public EventRegistration registerNameChangedHandler(
			final EventHandler<TypedEventArg<String>> handler) {
		return this.changeNameEvent.add(handler);
	}

	@Override
	public EventRegistration registerDescriptionChangeHandler(
			final EventHandler<TypedEventArg<String>> handler) {
	...
	@Override
	public EventRegistration regsiterChangeStateHandler(
			final EventHandler<TypedEventArg<StateType>> handler) {
