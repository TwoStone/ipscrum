...
public class ProjectHistoryView extends Composite implements IProjectHistoryView {
	...
	public ProjectHistoryView() {
		...
		this.initWidget(concreteProjectHistoryPanel);
		...
		final TextColumn<Incident> timeColumn = new TextColumn<Incident>() {
			...
			public String getValue(final Incident incident) {
				...
				if (incident.getStart().equals(incident.getEnd())) {
					...
					return fmt.format(incident.getStart()) + " - "
							+ fmt.format(incident.getEnd());
				}
			}
		};

		this.projectHistoryTable.addColumn(timeColumn,
				TextConstants.INCIDENT_TIME_COLUMN);
		...
		final TextColumn<Incident> descriptionColumn = new TextColumn<Incident>() {
			...
			public String getValue(final Incident incident) {
				if (incident.getDescription().length() <= 80) {
					...
					return incident.getDescription().substring(0, 80)
							+ TextConstants.POINTS;
				}
			}
		};
		this.projectHistoryTable
				.addColumn(descriptionColumn, TextConstants.DESCRIPTION);

		final TextColumn<Incident> personColumn = new TextColumn<Incident>() {
			@Override
			public String getValue(final Incident incident) {
				if (incident instanceof MultipleParticipantIncident) {
					return ((MultipleParticipantIncident) incident).getParticipants()
							.toString();
				} else {
		...
		this.projectHistoryTable.addColumn(personColumn,
		...
		this.createIncidentbtn.addClickHandler(new ClickHandler() {
			@Override
			public void onClick(final ClickEvent event) {
				ProjectHistoryView.this.createIncident.fire(ProjectHistoryView.this,
						null);
			}
		});
	 ...
	 * 
	 * @see
	 * fhdw.ipscrum.client.view.IProjectHistoryView#refreshProjectHistoryTable(java.util
	 * .Vector)
	 */
	@Override
	public void refreshProjectHistoryTable(final List<Incident> incidents) {
		...
		for (final Incident Incident : incidents) {
			final Element current =
					this.getProjectHistoryTable().getRowElement(counter);
	 ...
	 * 
	 * @see
	 * fhdw.ipscrum.client.view.IProjectHistoryView#addcreateIncidentHandler(fhdw.ipscrum
	 ...
	 * 
	 * @see
	 * fhdw.ipscrum.client.view.IProjectHistoryView#addcreateTypeHandler(fhdw.ipscrum.
	 * client.architecture.events.EventHandler)
	 */
	@Override
	 ...
	 * 
	 * @see
	 * fhdw.ipscrum.client.view.IProjectHistoryView#addchangeTypHandler(fhdw.ipscrum.client
	 ...
	 * 
	 * @see
	 * fhdw.ipscrum.client.view.IProjectHistoryView#addShowIncidentsHandler(fhdw.ipscrum
	 * .client.architecture.events.EventHandler)
	 */
	@Override
	...
	public List<IncidentType> getTypes() {
		return (List<IncidentType>) ((MultiSelectionModel<IncidentType>) this
				.getTypeCellList().getSelectionModel()).getSelectedSet();
	}
